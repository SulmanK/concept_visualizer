...
**Analysis:** This covers the initial page load effectively by splitting code based on routes. However, large components _within_ pages might still be loaded unnecessarily, especially modals or sections that aren't immediately visible or critical.
...
3.  **Implement Component-Level Lazy Loading:**
    ...
    - **Refactor Imports:** In the _parent_ component (e.g., `ConceptDetailPage.tsx`), change static imports of candidate components:
      ...
      // After:
      const EnhancedImagePreview = React.lazy(
    ...
    - **Wrap in `<Suspense>`:** Wrap the usage of the lazy-loaded component in a `<Suspense>` boundary within the parent component's render function, providing a suitable fallback (could be a simple `SkeletonLoader` or a smaller spinner).
      ...
      // ... inside parent component render ...
      {
        isModalOpen && (
          <Suspense
            fallback={<SkeletonLoader type="rectangle" height="400px" />}
          >
...
4.  **Testing:**
    ...
    - Use the Network tab in browser DevTools during page load and interaction to confirm that chunks for lazy-loaded components are loaded _only_ when needed.
...
1.  **Profile with React DevTools:**
    ...
    - **Analyze:** Identify components highlighted as re-rendering frequently, especially those whose props or state _haven't actually changed meaningfully_. Look for components high up the tree re-rendering unnecessarily.
...
3.  **Apply `useCallback` to Event Handlers Passed as Props:**
    ...
    - **Implementation Example (in RecentConceptsSection):**
      ...
      const handleEdit = useCallback(
        ...
        [navigate],
      ); // Add dependencies like navigate
...
3.  **Configure Props Correctly:**
    ...
    - **`width`/`height`:** Provide _intrinsic_ or _expected render_ dimensions if possible. This helps prevent layout shifts while the image loads. Use strings (e.g., `"100%"`) or numbers (pixels).
...
4.  **Implement Optimizations:**
    - **Dynamic `import()`:** As identified in the Lazy Loading section, apply dynamic imports not just at the route level but also for large/conditional components _within_ pages.
    - **Dependency Audit:**
