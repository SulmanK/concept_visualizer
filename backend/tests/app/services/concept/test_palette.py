...
class TestPaletteGenerator:
    ...
    @pytest.mark.asyncio
    async def test_generate_palettes_success(self, generator, mock_client):
        ...
        assert result[0]["colors"] == ["#0047AB", "#6495ED", "#00BFFF", "#87CEEB", "#B0E0E6"]
        assert result[0]["description"] == "A vibrant blue palette"

    @pytest.mark.asyncio
    async def test_generate_palettes_without_logo_description(self, generator, mock_client):
    ...
    @pytest.mark.asyncio
    async def test_generate_palettes_jigsawstack_error(self, generator, mock_client):
        ...
        with pytest.raises(JigsawStackError):
            await generator.generate_palettes(
                theme_description="A theme",
                logo_description="A logo"
            )

    @pytest.mark.asyncio
    async def test_generate_palettes_generic_error(self, generator, mock_client):
    ...
    @pytest.mark.asyncio
    async def test_generate_single_palette_success(self, generator, mock_client):
        ...
        # Call the method
        result = await generator.generate_single_palette(
            theme_description=theme_description,
            num_colors=num_colors
        )
        
        # Verify correct call was made to generate_color_palette
        ...
        assert result[0] == "#FF5733"
        assert result[1] == "#33FF57"

    @pytest.mark.asyncio
    async def test_generate_single_palette_jigsawstack_error(self, generator, mock_client):
        """Test generate_single_palette with JigsawStackError."""
        # Set up API client to raise a JigsawStackError
        mock_client.generate_color_palette.side_effect = JigsawStackError("API Error")
        
        # Call the method and check that it raises the original JigsawStackError
        with pytest.raises(JigsawStackError):
            await generator.generate_single_palette(
                theme_description="A theme"
            )

    @pytest.mark.asyncio
    async def test_generate_single_palette_generic_error(self, generator, mock_client):
        """Test generate_single_palette with a generic error."""
        # Set up API client to raise a generic exception
        mock_client.generate_color_palette.side_effect = Exception("Some generic error")
