#!/usr/bin/env python
"""Utility script to generate a mock CloudEvent for testing refinement tasks.

This script:
1. Reads the mock_refinement_payload.json file
2. Base64 encodes its contents
3. Creates a CloudEvent structure with the encoded payload
4. Writes the CloudEvent to mock_refinement_cloudevent.json
"""
...
with open(current_dir / "mock_refinement_payload.json", "r") as f:
    payload = f.read()
...
with open(current_dir / "mock_refinement_cloudevent.json", "w") as f:
    json.dump(cloud_event, f, indent=2)

print(f"CloudEvent written to {current_dir / 'mock_refinement_cloudevent.json'}")
...
print("curl -X POST http://localhost:8080 " '-H "Content-Type: application/cloudevents+json" ' f"-d @{current_dir / 'mock_refinement_cloudevent.json'}")
