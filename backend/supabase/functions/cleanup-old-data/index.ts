...
async function cleanupStuckTasks(minutesThreshold: number): Promise<number> {
  try {
    console.log(`Looking for stuck tasks older than ${minutesThreshold} minutes...`);
    
    // Calculate the cutoff timestamp (current time minus threshold)
    const now = new Date();
    const cutoffTime = new Date(now.getTime() - (minutesThreshold * 60 * 1000)).toISOString();
    ...
    for (const task of stuckTasks) {
      ...
      if (updateError) {
        ...
        updatedCount++;
        console.log(`Marked task ${task.id} as failed (was in '${task.status}' state since ${task.updated_at})`);
      }
    }
