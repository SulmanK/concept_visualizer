...
def setup_logging(log_level: str = "INFO", log_format: Optional[str] = None, log_dir: str = "logs") -> None:
    ...
    try:
        ...
        file_handler = logging.handlers.RotatingFileHandler(app_log_file, maxBytes=10 * 1024 * 1024, backupCount=5, encoding="utf-8")  # 10 MB  # Keep 5 backup files max
        file_handler.setFormatter(formatter)
        root_logger.addHandler(file_handler)

        # Error log - separate file for errors and above
        error_log_file = log_directory / "error.log"
        error_file_handler = logging.handlers.RotatingFileHandler(error_log_file, maxBytes=10 * 1024 * 1024, backupCount=5, encoding="utf-8")
...
def is_health_check(path: str) -> bool:
    ...
    """
    health_check_paths = ["/health", "/api/health", "/ping", "/api/ping", "/_health"]
