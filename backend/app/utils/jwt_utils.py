...
# Add a comment to indicate that types-python-jose needs to be installed
# Need to run: uv add types-python-jose
...
def create_supabase_jwt(user_id: str, expiry_seconds: int = 259200) -> str:
    ...
    try:
        ...
        # Ensure we return a string (jwt.encode might return str or bytes depending on version)
        if isinstance(token, bytes):
            return token.decode("utf-8")  # Convert bytes to string
        else:
            token_str: str = token  # Type annotation to help mypy
            return token_str
...
def verify_jwt(token: str) -> Dict[str, Any]:
    ...
    try:
        ...
        # Explicitly return the claims as Dict[str, Any] to avoid mypy error
        return dict(claims)
...
def extract_user_id_from_token(token: str, validate: bool = True) -> Optional[str]:
    ...
    try:
        if validate:
            ...
            claims_result = decode_token(token)
        ...
        if "user_id" in claims:
            ...
            return str(claims["user_metadata"]["user_id"])
...
def create_supabase_jwt_for_storage(path: str, expiry_timestamp: int) -> str:
    ...
    }  # Storage path  # Issued at  # Expiration timestamp
...
def decode_token(token: str) -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        # Explicitly return a dictionary to avoid mypy error
        return dict(payload)
    except Exception as e:
