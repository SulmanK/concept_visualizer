...
# Import TaskNotFoundError at the top level
...
class UnprocessableEntityError(APIError):
    ...
    def __init__(
        ...
        # Convert dict detail to string if needed for parent class
...
class ServiceUnavailableError(APIError):
    ...
    def __init__(
        ...
        Args:
            detail: Error message
            retry_after: Seconds until the service is expected to be back
            headers: Additional headers
        """
        headers = headers or {}
        if retry_after is not None:
...
class GatewayTimeoutError(APIError):
    ...
    def __init__(self, detail: str = "Gateway timeout", headers: Optional[Dict[str, str]] = None):
        """Initialize gateway timeout error."""
        super().__init__(status_code=status.HTTP_504_GATEWAY_TIMEOUT, detail=detail, headers=headers)
...
def map_application_error_to_api_error(error: ApplicationError) -> APIError:
    ...
    from app.core.exceptions import (
        ServiceUnavailableError as AppServiceUnavailableError,  # Authentication and authorization errors; Resource errors; Validation errors; Rate limiting; Service availability; Database errors; Integration errors; Processing errors; Concept errors; Configuration errors
    )
    from app.core.exceptions import SessionNotFoundError, StorageError, StorageOperationError, ValidationError
    ...
    if isinstance(error, ExternalServiceError):
        ...
        return ServiceUnavailableError(detail=f"{service_name} service is currently unavailable: {detail}")
...
def configure_error_handlers(app: Any) -> None:
    ...
    @app.exception_handler(APIError)  # type: ignore[misc]
    ...
    async def http_exception_handler(request: Request, exc: StarletteHTTPException) -> JSONResponse:
        ...
        # Log client errors with warning level
            ...
            logger.warning(f"Client HTTP error: {exc.status_code} - {exc.detail}")
    ...
    async def application_error_handler(request: Request, exc: ApplicationError) -> JSONResponse:
        ...
        if api_error.status_code >= 500:
            ...
            logger.warning(f"Application error mapped to {api_error.status_code}: {exc.message}")

        # Include error details if available
        content: Dict[str, Any] = {"detail": api_error.detail}
        if hasattr(exc, "details") and exc.details:
            # Filter out sensitive information
            safe_details = {k: v for k, v in exc.details.items() if k not in ["token", "api_key", "password", "secret"]}
            if safe_details:
    ...
    @app.exception_handler(TaskNotFoundError)  # type: ignore[misc]
