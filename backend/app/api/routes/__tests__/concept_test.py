...
def mock_jigsawstack_client():
    ...
    """
    with mock.patch(
        "backend.app.services.concept_service.get_jigsawstack_client"
    ) as mocked_factory:
...
@pytest.mark.asyncio
async def test_generate_concept(mock_jigsawstack_client, sample_generation_response):
    """
    ...
    mock_jigsawstack_client.generate_image.return_value = (
        sample_generation_response["image_url"]
    )
...
@pytest.mark.asyncio
async def test_generate_concept_with_invalid_inputs(mock_jigsawstack_client):
    """
...
@pytest.mark.asyncio
async def test_generate_concept_handles_service_errors(mock_jigsawstack_client):
    """
    Test that the generate concept endpoint handles service errors properly.
    """
    # Mock service to raise an exception
    mock_jigsawstack_client.generate_image.side_effect = Exception(
...
@pytest.mark.asyncio
async def test_refine_concept(mock_jigsawstack_client, sample_generation_response):
    """
    Test concept refinement endpoint with valid input.
    """
    # Mock the refine_image method
    mock_jigsawstack_client.refine_image.return_value = (
        sample_generation_response["image_url"]
    )
...
@pytest.mark.asyncio
async def test_refine_concept_with_invalid_inputs(mock_jigsawstack_client):
    """
...
@pytest.mark.asyncio
async def test_refine_concept_handles_service_errors(mock_jigsawstack_client):
    """
    Test that the refine concept endpoint handles service errors properly.
    """
    # Mock service to raise an exception
    mock_jigsawstack_client.refine_image.side_effect = Exception(
        "API Connection Error"
    )
