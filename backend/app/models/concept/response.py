...
class ColorPalette(APIBaseModel):
    ...
    additional_colors: List[str] = Field(default=[], description="Additional color options")
...
class GenerationResponse(APIBaseModel):
    ...
    variations: List[PaletteVariation] = Field(default=[], description="List of palette variations with distinct images")

    # Optional fields for refinement responses
    original_image_url: Optional[HttpUrl] = Field(None, description="URL of the original image (for refinements)")
    refinement_prompt: Optional[str] = Field(None, description="Prompt used for refinement")
...
class ConceptSummary(APIBaseModel):
    ...
    color_variations: List[PaletteVariation] = Field(default=[], description="List of all color variations with their images")


class ConceptDetail(ConceptSummary):
    """Detailed information about a concept including all variations."""

    variations: List[PaletteVariation] = Field(default=[], description="List of all color variations with their images")
