...
test('concept generation flow', async ({ page, mockApi }) => {
  ...
  await expect(page.getByRole('heading', { name: /Create Visual Concepts/ })).toBeVisible();
  
  // Fill out the form
  await page.getByLabel(/Logo Description/i).fill('Modern tech logo with abstract shapes');
  await page.getByLabel(/Theme Description/i).fill('Gradient indigo color scheme with clean design');
  ...
  await expect(page.getByText(/Generating your concept/i)).not.toBeVisible({ timeout: 10000 });
  ...
  await expect(conceptImage).toHaveAttribute('src', 'https://example.com/mock-concept.png');
  
  // Verify the color palette is displayed
  await expect(page.getByText(/Color Palette/i)).toBeVisible();
  
  // Verify action buttons are present
  await expect(page.getByRole('button', { name: /Refine This Concept/i })).toBeVisible();
