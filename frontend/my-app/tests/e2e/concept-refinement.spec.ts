...
test('concept refinement flow', async ({ page, mockApi, mockLocalStorage }) => {
  ...
  await expect(page.getByRole('heading', { name: /Refine Your Concept/i })).toBeVisible();
  ...
  await expect(originalImage).toHaveAttribute('src', 'https://example.com/original-concept.png');
  
  // Fill out the refinement form
  await page.getByLabel(/Refinement Instructions/i).fill('Make it more vibrant with brighter colors');
  ...
  await expect(page.getByText(/Refining your concept/i)).not.toBeVisible({ timeout: 10000 });
  ...
  await expect(refinedImage).toHaveAttribute('src', 'https://example.com/refined-concept.png');
