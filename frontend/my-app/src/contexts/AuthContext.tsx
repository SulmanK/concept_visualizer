...
export const AuthProvider: React.FC<{children: React.ReactNode}> = ({ children }) => {
    ...
    const initAuth = async () => {
      ...
      try {
        ...
        if (authSession) {
          ...
          console.log('[AUTH:Context] Initial user anonymous status:', currentIsAnonymous);
        } else {
          ...
          async (event, newSession) => {
            ...
            if (newSession) {
              ...
              if (currentUserId === prevUserId && currentIsAnonymous === prevIsAnonymous) {
                console.log('[AUTH:Context] Session updated (token likely refreshed), user identity unchanged.');
              }
            } else {
              setSession(null);
            }
            ...
            if (isSignificantChange) {
              console.log(`[AUTH:Context] Significant auth change detected (Event: ${event}, User Change: ${currentUserId !== prevUserId}, Anon Change: ${currentIsAnonymous !== prevIsAnonymous}). Updating user state.`);
  ...
  const handleLinkEmail = useCallback(async (email: string): Promise<boolean> => {
  ...
  }), [session, user, isAnonymous, isLoading, error, handleSignOut, handleLinkEmail]);
...
export const useIsAnonymous = () => {
  const context = useContextSelector(AuthContext, state => state?.isAnonymous);
  if (context === undefined) {
