...
export const RateLimitsPanel: React.FC<RateLimitsPanelProps> = ({
  ...
  const handleRefresh = useCallback(() => {
    ...
    refetch().then((result) => {
      if (result.isSuccess) {
        console.log(
          "RateLimitsPanel: Manual refresh completed successfully",
          result.data,
        );
      } else {
  ...
  const renderRateLimitItem = (label: string, category: RateLimitCategory) => {
    ...
    const percentage = limit
      ? Math.min(100, (remaining / parseInt(limit.split("/")[0], 10)) * 100)
      : 0;
    ...
    return (
      <Box key={category} sx={{ mb: 2 }}>
              ...
              <Tooltip title="Active">
                <div
                  style={{
                    ...
                    marginRight: "4px",
                    animation:
                      remaining <= 0 ? "pulse 2s infinite ease-in-out" : "none",
                  }}
                />
              </Tooltip>
            )}
        ...
        <Tooltip title={`Resets in ${resetTime}`} placement="top">
          <Box
            sx={{
              position: "relative",
              height: 8,
      ...
      >
        ...
        {showRefresh && (
          <Tooltip
            title={
              refreshCooldown > 0
                ? `Refresh again in ${refreshCooldown}s`
                : "Refresh rate limits"
            }
          >
            <span>
