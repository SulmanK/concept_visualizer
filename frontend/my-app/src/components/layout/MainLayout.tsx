...
export const MainLayout: React.FC<MainLayoutProps> = ({ className = "" }) => {
  ...
  return (
    <div className="flex flex-col min-h-screen w-full m-0 p-0 bg-gradient-to-br from-indigo-50 to-indigo-200">
      ...
      <main
        className={`flex-1 w-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4 sm:py-6 md:py-8 ${className}`}
      >
        <Outlet />
      </main>
      ...
      <div className="fixed right-0 top-1/2 transform -translate-y-1/2 z-50">
        <div className="flex">
          ...
          <button
            ...
            title={
              showRateLimits ? "Hide API usage limits" : "Show API usage limits"
            }
          >
            {showRateLimits ? (
              <svg
