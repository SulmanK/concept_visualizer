...
export const OfflineStatus: React.FC<OfflineStatusProps> = ({
  ...
  const { isOnline, isSlowConnection, connectionType, checkConnection, offlineSince } = networkStatus;
  ...
  return (
    <div
      className={`
        ...
        ${!isVisible ? 'translate-y-[-100%] opacity-0' : 'translate-y-0 opacity-100'}
        ${isOnline ? 'bg-yellow-50 text-yellow-800 border-yellow-200' : 'bg-red-50 text-red-800 border-red-200'}
      ...
      <div className="flex items-center">
        ...
        <span className="text-sm font-medium">
          ...
          {!isOnline && offlineSince && (
            <span className="ml-1 text-xs opacity-80">{getOfflineDuration()}</span>
          )}
        </span>
