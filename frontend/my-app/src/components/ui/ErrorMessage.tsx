...
export const ErrorMessage: React.FC<ErrorMessageProps> = ({
  ...
  const isValidation = type === 'validation' && validationErrors && Object.keys(validationErrors).length > 0;
        ...
        {isRateLimit && (
          <div className="mt-3 bg-white bg-opacity-50 p-3 rounded-md">
            ...
            <div className="flex justify-between text-xs">
              <span>Reset:</span>
              <span className="font-medium">At the beginning of next month</span>
            </div>
            ...
            <div className="w-full bg-gray-200 rounded-full h-1.5 my-2">
              <div 
                ...
                style={{ 
                  width: `${Math.min(100, (rateLimitData.current / rateLimitData.limit) * 100)}%` 
                }}
              ></div>
            </div>
          </div>
...
export const RateLimitErrorMessage: React.FC<Omit<ErrorMessageProps, 'type' | 'rateLimitData' | 'message'> & { error: ErrorWithCategory }> = ({
  ...
  if (!error.limit || !error.resetAfterSeconds) {
    console.warn('RateLimitErrorMessage received an error without rate limit data', error);
  }
 ...
 */
export const ValidationErrorMessage: React.FC<Omit<ErrorMessageProps, 'type' | 'validationErrors' | 'message'> & { 
