 ...
 */
export const mapEndpointToCategory = (
  endpoint: string,
...
export const extractRateLimitHeaders = (
  ...
  if (limit && remaining && reset) {
    ...
    if (category) {
      console.log(
        `Updating rate limit for category: ${category} with remaining: ${remaining}`,
      );

      // Directly update the React Query cache
      queryClient.setQueryData<RateLimitsResponse>(
        ["rateLimits"],
        (oldData) => {
          // If we don't have existing data in the cache, we can't update it
          if (!oldData || !oldData.limits || !oldData.limits[category]) {
            console.warn(
              "Cannot update rate limit cache: no existing data for category",
              category,
            );
            return oldData;
          }
          ...
          console.log(
            `[RateLimitService] Updated rate limit cache for category: ${category}, remaining: ${parsedRemaining}`,
          );
    ...
    if (endpoint && endpoint.includes("/export/process")) {
      console.warn(
        `Expected rate limit headers from ${endpoint} but didn't receive them:`,
        {
          hasLimit: !!limit,
...
const hasCachedRateLimits = (): boolean => {
  return !!rateLimitCache.main && rateLimitCache.main.expiresAt > Date.now();
};
...
export const getRateLimitInfoForCategory = (
  ...
  const cachedData = queryClient.getQueryData<RateLimitsResponse>([
    "rateLimits",
  ]);
  ...
  if (
    rateLimitCache.main?.limits?.limits &&
    rateLimitCache.main.limits.limits[category]
  ) {
...
export const decrementRateLimit = (
  ...
  if (!rateLimit) {
    console.warn(
      `Cannot decrement rate limit for ${category}: no rate limit info available`,
    );
    return null;
  }
  ...
  console.log(
    `Decremented rate limit for ${category} by ${amount}`,
    updatedRateLimit,
  );
...
export const fetchRateLimits = async (
  ...
  try {
    ...
    const response = await apiClient.get<RateLimitsResponse>(
    ...
    const errorResponse: RateLimitsResponse = {
      ...
      limits: {
        generate_concept: {
          ...
          remaining: 0,
          reset_after: 0,
          error: "Error fetching rate limits",
        },
        refine_concept: {
          limit: "error",
          remaining: 0,
