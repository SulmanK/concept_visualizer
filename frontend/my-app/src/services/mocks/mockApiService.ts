...
export class MockApiService {
   ...
   */
  private async simulateResponse<T>(successData: T, errorData: ApiError): Promise<ApiResponse<T>> {
    await this.delay();
   ...
   */
  async generateConcept(request: PromptRequest): Promise<ApiResponse<GenerationResponse>> {
   ...
   */
  async refineConcept(request: RefinementRequest): Promise<ApiResponse<GenerationResponse>> {
    // Validate inputs (similar to real implementation)
 ...
 */
export function createMockApiResponse<T>(data?: T, error?: ApiError): ApiResponse<T> {
  return {
    data,
    error,
    loading: false
