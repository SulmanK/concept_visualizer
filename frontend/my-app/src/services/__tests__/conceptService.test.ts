...
describe('Concept Service', () => {
  ...
  describe('fetchRecentConceptsFromApi', () => {
    ...
    it('should handle API errors', async () => {
      ...
      await expect(fetchRecentConceptsFromApi('test-user-id'))
    ...
    it('should log detailed information about color variations', async () => {
      ...
      expect(consoleLogSpy).toHaveBeenCalledWith(
        expect.stringContaining('Concept 1/2 (ID: concept-1): 3 color variations')
      );
      expect(consoleLogSpy).toHaveBeenCalledWith(
        expect.stringContaining('Concept 2/2 (ID: concept-2): 0 color variations')
      );
      expect(consoleLogSpy).toHaveBeenCalledWith(
  ...
  describe('fetchConceptDetailFromApi', () => {
    it('should call the API with correct concept ID', async () => {
      ...
      // Verify logs
      expect(consoleLogSpy).toHaveBeenCalledWith(
        expect.stringContaining('Fetching concept detail for ID test-concept-id')
      );
      expect(consoleLogSpy).toHaveBeenCalledWith(
    ...
    it('should throw for non-404 errors', async () => {
      ...
      // Call the function and expect it to throw
      await expect(fetchConceptDetailFromApi('test-concept-id'))
