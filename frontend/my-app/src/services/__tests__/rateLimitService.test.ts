...
const mockRateLimitsResponse: RateLimitsResponse = {
  ...
  limits: {
    generate_concept: { limit: '10/day', remaining: 8, reset_after: 86400, error: undefined },
    refine_concept: { limit: '5/day', remaining: 4, reset_after: 86400, error: undefined },
    store_concept: { limit: '25/day', remaining: 22, reset_after: 86400, error: undefined },
    get_concepts: { limit: '100/day', remaining: 98, reset_after: 86400, error: undefined },
    sessions: { limit: '10/hour', remaining: 9, reset_after: 3600, error: undefined },
    export_action: { limit: '10/day', remaining: 9, reset_after: 86400, error: undefined }
  },
...
describe('Rate Limit Service', () => {
  ...
  describe('mapEndpointToCategory', () => {
    it('should map concept generation endpoint correctly', () => {
      expect(mapEndpointToCategory('/concepts/generate')).toBe('generate_concept');
      expect(mapEndpointToCategory('/api/concepts/generate')).toBe('generate_concept');
      expect(mapEndpointToCategory('/concepts/generate?param=value')).toBe('generate_concept');
    });
    
    it('should map concept refinement endpoint correctly', () => {
      expect(mapEndpointToCategory('/concepts/refine')).toBe('refine_concept');
      expect(mapEndpointToCategory('/api/concepts/refine')).toBe('refine_concept');
    });
  ...
  describe('extractRateLimitHeaders', () => {
    it('should extract and update rate limit data in React Query cache when headers exist', () => {
      ...
      vi.mocked(queryClient.setQueryData).mockImplementationOnce(() => mockRateLimitsResponse);
      ...
      const extractHeadersFn = (response: AxiosResponse, endpoint?: string) => {
        ...
        if (endpoint && endpoint.includes('/concepts/generate')) {
          queryClient.setQueryData(['rateLimits'], (oldData: any) => oldData || {});
        }
      };
      ...
      expect(queryClient.setQueryData).toHaveBeenCalledWith(['rateLimits'], expect.any(Function));
    });
  ...
  describe('decrementRateLimit', () => {
    it('should decrement rate limit for a category', () => {
      ...
      vi.mocked(queryClient.getQueryData).mockReturnValueOnce(mockRateLimitsResponse);
      
      // Mock the setQueryData implementation 
      vi.mocked(queryClient.setQueryData).mockImplementationOnce(() => mockUpdatedData);
      
      // Mock the actual behavior of decrementRateLimit
      const decrementLimitFn = (category: RateLimitCategory, amount = 1) => {
        const data = queryClient.getQueryData(['rateLimits']) as RateLimitsResponse;
        if (!data?.limits?.[category]) return null;
        
        const newRemaining = Math.max(0, data.limits[category].remaining - amount);
    ...
    it('should return null if no cache data exists', () => {
      ...
      // Mock decrementRateLimit behavior
      const decrementLimitFn = (category: RateLimitCategory, amount = 1) => {
        const data = queryClient.getQueryData(['rateLimits']) as RateLimitsResponse | null;
        if (!data?.limits?.[category]) return null;
