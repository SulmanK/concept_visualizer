...
export const ExportOptions: React.FC<ExportOptionsProps> = ({
  ...
  const componentId = useRef(`export-options-${Math.random().toString(36).substr(2, 9)}`);
  ...
  const safeRevokeObjectURL = (url: string) => {
    ...
    if (!revokedUrlsRef.current.has(url)) {
      console.log(`[ExportOptions ${componentId.current}] Revoking blob URL:`, url);
      URL.revokeObjectURL(url);
      revokedUrlsRef.current.add(url);
    } else {
      console.log(`[ExportOptions ${componentId.current}] URL already revoked, skipping:`, url);
    }
  };
  ...
  useEffect(() => {
    if (exportError) {
      console.log(`[ExportOptions ${componentId.current}] Export error detected:`, exportError);
      ...
      console.log(`[ExportOptions ${componentId.current}] Preview error detected:`, previewError);
  ...
  useEffect(() => {
    return () => {
      console.log(`[ExportOptions ${componentId.current}] Component unmounting, cleaning up resources`);
  ...
  const determineStorageBucket = useCallback((): string => {
    console.log(`[ExportOptions ${componentId.current}] Determining bucket: isPaletteVariation=${isPaletteVariation}, storagePath=${storagePath}, variationName=${variationName}`);
    
    // If isPaletteVariation is explicitly set, always use palette bucket
    if (isPaletteVariation) {
      console.log(`[ExportOptions ${componentId.current}] Using palette-images bucket: isPaletteVariation flag is true`);
    ...
    if (variationName && (
      ...
      console.log(`[ExportOptions ${componentId.current}] Using palette-images bucket: variation name indicates palette`);
      return 'palette-images';
    }
  ...
  const forceClearMutationState = useCallback(() => {
    console.log(`[ExportOptions ${componentId.current}] Forcing clear of mutation state`);
    resetPreviewMutation();
    // Wait a small amount of time and check if state is actually reset
    setTimeout(() => {
      console.log(`[ExportOptions ${componentId.current}] After reset: isPreviewExporting=${isPreviewExporting}`);
    }, 100);
  ...
  const handlePreview = async () => {
    ...
    if (isPreviewExporting) {
      console.log(`[ExportOptions ${componentId.current}] WARNING: Preview already in progress, forcibly resetting state`);
      resetPreviewMutation();
    ...
    const timeoutId = setTimeout(() => {
      console.log(`[ExportOptions ${componentId.current}] Preview request timed out, aborting`);
    ...
    try {
      ...
      console.log(`[ExportOptions ${componentId.current}] Using bucket "${bucket}" for preview of image: ${imagePath}`);
      ...
      previewImage({
        ...
        onSuccess: (blob) => {
          console.log(`[ExportOptions ${componentId.current}] Preview request ${requestId} succeeded, creating blob URL`);
        ...
        onError: (error) => {
          console.error(`[ExportOptions ${componentId.current}] Preview request ${requestId} failed:`, error);
        ...
        onSettled: () => {
          console.log(`[ExportOptions ${componentId.current}] Preview request ${requestId} settled. Explicitly resetting state.`);
          ...
          setTimeout(() => {
            ...
            setTimeout(() => {
              ...
              if (isPreviewExporting) {
                console.log(`[ExportOptions ${componentId.current}] Still loading after reset, forcing another reset`);
                resetPreviewMutation();
              }
            }, 50);
          }, 50);
  ...
  const handleDownloadClick = () => {
    ...
    console.log(`[ExportOptions ${componentId.current}] Download button clicked`);
    console.log(`[ExportOptions ${componentId.current}] Image URL:`, imageUrl);
    console.log(`[ExportOptions ${componentId.current}] Storage path:`, storagePath);
    console.log(`[ExportOptions ${componentId.current}] Is palette variation:`, isPaletteVariation);
    console.log(`[ExportOptions ${componentId.current}] Variation name:`, variationName);
    ...
    try {
      ...
      // Determine which bucket the image is in
      const bucket = determineStorageBucket();
      console.log(`[ExportOptions ${componentId.current}] Using bucket "${bucket}" for image: ${imagePath}`);
      ...
      const sanitizedTitle = conceptTitle.replace(/[^\w\s-]/g, '').trim().replace(/\s+/g, '_');
      const sanitizedVariation = variationName ? variationName.replace(/[^\w\s-]/g, '').trim().replace(/\s+/g, '_') : '';
      
      // Construct the filename with the pattern: concept-variation-size.format
      const filename = `${sanitizedTitle}${sanitizedVariation ? `-${sanitizedVariation}` : ''}-${selectedSize}.${selectedFormat}`;
      
      console.log(`[ExportOptions ${componentId.current}] Downloading file as: ${filename}`);
      console.log(`[ExportOptions ${componentId.current}] Request params:`, {
        imageIdentifier: imagePath,
        format: selectedFormat,
        size: selectedSize,
  ...
  const handleCopyLink = () => {
    ...
    try {
      ...
      setErrorMessage(`Failed to copy link: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  };
