...
vi.mock("../components/RefinementHeader", () => ({
  RefinementHeader: () => (
    <div data-testid="refinement-header">Refine Your Concept</div>
  ),
}));
...
vi.mock("../components/RefinementForm", () => ({
  RefinementForm: ({ onSubmit }) => (
    <div data-testid="refinement-form">
      <button
        onClick={() =>
          onSubmit("Make it blue", "Updated logo", "Updated theme", ["colors"])
        }
      >
        Submit Form
      </button>
...
vi.mock("../components/ComparisonView", () => ({
  ComparisonView: () => (
    <div data-testid="comparison-view">Comparison View</div>
  ),
}));
...
describe("RefinementPage Component", () => {
  beforeEach(() => {
    ...
    vi.mocked(
      useConceptMutationsModule.useRefineConceptMutation,
    ).mockReturnValue({
      mutate: vi.fn(),
      reset: vi.fn(),
      isPending: false,
