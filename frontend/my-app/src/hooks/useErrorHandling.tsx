...
export const useErrorHandling = (
  ...
  const setError = useCallback(
    (
      ...
      if (showToasts) {
        ...
        const toastType =
          category === "server" || category === "client"
            ? "error"
            : category === "validation"
            ? "warning"
            : category === "network"
  ...
  const categorizeError = useCallback((error: unknown): ErrorCategory => {
    ...
    if (error instanceof NetworkError) {
      ...
      if (error.possibleRateLimit) {
        console.log(
          "[useErrorHandling] Detected NetworkError that might be a rate limit issue",
        );
        return "rateLimit";
      }
      return "network";
    ...
    (error: unknown) => {
      ...
      if (category === "rateLimit") {
        if (error instanceof RateLimitError) {
          ...
          errorWithCategory.resetAfterSeconds =
            err.resetAfterSeconds ||
  ...
  const showErrorToast = useCallback(() => {
    if (error) {
      // Map error category to toast type
      const toastType =
        error.category === "server" || error.category === "client"
          ? "error"
