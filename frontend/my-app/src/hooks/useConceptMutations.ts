...
export function useGenerateConceptMutation() {
  ...
  return useMutation<TaskResponse, Error, PromptRequest>({
    ...
    mutationFn: async (data) => {
      ...
      if (!networkStatus.isOnline) {
        const error = new Error(
          "You are offline. Cannot generate concept without an internet connection.",
        );
      ...
      try {
        ...
        const initiatingTimeout = setTimeout(() => {
          console.log(
            "[useGenerateConceptMutation] API call timeout, clearing initiating state",
          );
        ...
        const response = await apiClient.post<TaskResponse>(
          API_ENDPOINTS.GENERATE_CONCEPT,
          data,
        );
        ...
        console.log(
          `[useGenerateConceptMutation] Setting active task ID: ${taskId}`,
        );
        ...
        if (response.data.result_id) {
          console.log(
            `[useGenerateConceptMutation] Initial response already has result_id: ${response.data.result_id}`,
          );
        ...
        console.error(
          "[useGenerateConceptMutation] Error during fetch:",
          error,
        );
    ...
    onError: (error) => {
      console.error(
        "[useGenerateConceptMutation] Error during generation:",
        error,
      );
      ...
      // Special case for network errors that might be rate limits
        ...
        console.warn(
          "[useGenerateConceptMutation] Detected network error that might be a rate limit issue",
        );
      ...
      // Check for specific error messages that might indicate rate limits
        ...
        console.warn(
          "[useGenerateConceptMutation] Detected error message that might indicate a rate limit issue",
        );
    ...
    onSettled: () => {
      ...
      setTimeout(() => {
        queryClient.removeQueries({
          queryKey: queryKeys.mutations.conceptGeneration(),
        });
      }, 300);
    },
  });
...
export function useRefineConceptMutation() {
  ...
  return useMutation<TaskResponse, Error, RefinementRequest>({
    ...
    mutationFn: async (data) => {
      ...
      if (!networkStatus.isOnline) {
        const error = new Error(
          "You are offline. Cannot refine concept without an internet connection.",
        );
        errorHandler.setError(
          error.message,
          "network",
      ...
      try {
        ...
        const initiatingTimeout = setTimeout(() => {
          console.log(
            "[useRefineConceptMutation] API call timeout, clearing initiating state",
          );
          setIsTaskInitiating(false);
        }, 10000); // 10 seconds should be more than enough for the API to respond
        ...
        // Set the task ID in the global context
        console.log(
          `[useRefineConceptMutation] Setting active task ID: ${taskId}`,
        );
        setActiveTask(taskId);
        ...
        if (response.data.result_id) {
          console.log(
            `[useRefineConceptMutation] Initial response already has result_id: ${response.data.result_id}`,
          );
          setLatestResultId(response.data.result_id);
        }
    ...
    onError: (error) => {
      console.error(
        "[useRefineConceptMutation] Error during refinement:",
        error,
      );
      ...
      // Special case for network errors that might be rate limits
        ...
        console.warn(
          "[useRefineConceptMutation] Detected network error that might be a rate limit issue",
        );
      ...
      // Check for specific error messages that might indicate rate limits
        ...
        console.warn(
          "[useRefineConceptMutation] Detected error message that might indicate a rate limit issue",
        );
    ...
    onSettled: () => {
      ...
      setTimeout(() => {
        queryClient.removeQueries({
          queryKey: queryKeys.mutations.conceptRefinement(),
        });
      }, 300);
    },
  });
