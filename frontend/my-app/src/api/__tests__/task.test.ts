  ...
  describe("fetchTaskStatus", () => {
    it("should call apiClient.get with the correct URL", async () => {
      ...
      expect(apiClient.get).toHaveBeenCalledWith(
        API_ENDPOINTS.TASK_STATUS_BY_ID(mockTaskId),
      );
    ...
    it("should throw an error when no taskId is provided", async () => {
      ...
      await expect(fetchTaskStatus("")).rejects.toThrow(
        "No task ID provided for status check",
      );
    });
  });
  ...
  describe("cancelTask", () => {
    it("should call apiClient.post with the correct URL", async () => {
      ...
      expect(apiClient.post).toHaveBeenCalledWith(
        API_ENDPOINTS.TASK_CANCEL(mockTaskId),
        {},
      );
    ...
    it("should throw an error when no taskId is provided", async () => {
      // Call the function with an empty string
      await expect(cancelTask("")).rejects.toThrow(
        "No task ID provided for cancellation",
      );
    });
  });
});
