...
export const ConceptProvider: React.FC<{ children: ReactNode }> = ({
  children,
}) => {
    ...
    try {
      ...
      setError(
        err instanceof Error
          ? err
          : new Error("Failed to load recent concepts"),
      );
  ...
  const generateConcept = async (input: PromptInput): Promise<void> => {
    if (!validateInput(input)) {
      setState({
        isLoading: false,
        error: new Error(
          "Please provide both logo and theme descriptions (minimum 3 characters each)",
        ),
        result: null,
      });
      return;
      ...
      setState({
        ...
        error:
          error instanceof Error
            ? error
            : new Error("Failed to generate concept"),
        result: null,
      });
    }
  };
...
export const RecentConcepts: React.FC = () => {
  const { recentConcepts, isLoading, error, refreshRecentConcepts } =
    useConcepts();
...
.conceptCard:hover {
  ...
  box-shadow:
    0 10px 25px -5px rgba(67, 56, 202, 0.1),
    0 10px 10px -5px rgba(67, 56, 202, 0.04);
}
  ...
  useEffect(() => {
    async function loadConceptDetail() {
      ...
      try {
        ...
        setError(
          err instanceof Error ? err : new Error("Failed to load concept"),
        );
      } finally {
        setIsLoading(false);
      }
    ...
    <div className={styles.container}>
      ...
      <div className={styles.content}>
        ...
        <section className={styles.actions}>
          <Link to={`/refine/${concept.id}`} className="btn btn-primary">
            Refine This Concept
          </Link>
