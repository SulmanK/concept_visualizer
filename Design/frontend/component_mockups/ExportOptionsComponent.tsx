    ...
    <div className="bg-white/90 backdrop-blur-sm rounded-xl shadow-md p-6 mb-8 border border-indigo-100">
      ...
      <div className="flex flex-col md:flex-row gap-6">
        ...
        <div className="w-full md:w-2/3">
          <div className="mb-6">
            <h4 className="text-sm font-semibold text-indigo-700 mb-2">Select Format</h4>
            <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
              ...
              <div 
                className={`border rounded-lg p-4 bg-white hover:shadow-md transition-all cursor-pointer group relative flex flex-col items-center
                  ${selectedFormat === 'png' ? 'border-indigo-400 shadow-md' : 'border-indigo-200 hover:border-indigo-400'}`}
                onClick={() => setSelectedFormat('png')}
              >
                <div className="text-indigo-600 font-bold text-lg">.PNG</div>
                <span className="text-xs text-gray-500 mb-2">Transparent Background</span>
                <div className="bg-indigo-100 text-indigo-800 text-xs px-2 py-1 rounded-full">Recommended</div>
                <div className="mt-2 text-xs text-gray-600 text-center">Best for web, digital use</div>
              </div>
              
              {/* SVG Format */}
              <div 
                className={`border rounded-lg p-4 bg-white hover:shadow-md transition-all cursor-pointer group relative flex flex-col items-center
                  ${selectedFormat === 'svg' ? 'border-indigo-400 shadow-md' : 'border-indigo-200 hover:border-indigo-400'}`}
                onClick={() => setSelectedFormat('svg')}
              >
                <div className="text-indigo-600 font-bold text-lg">.SVG</div>
                <span className="text-xs text-gray-500 mb-2">Vector Format</span>
                <div className="bg-indigo-100 text-indigo-800 text-xs px-2 py-1 rounded-full">Scalable</div>
                <div className="mt-2 text-xs text-gray-600 text-center">Best for printing, scaling</div>
              </div>
              
              {/* JPG Format */}
              <div 
                className={`border rounded-lg p-4 bg-white hover:shadow-md transition-all cursor-pointer group relative flex flex-col items-center
                  ${selectedFormat === 'jpg' ? 'border-indigo-400 shadow-md' : 'border-indigo-200 hover:border-indigo-400'}`}
                onClick={() => setSelectedFormat('jpg')}
              >
                <div className="text-indigo-600 font-bold text-lg">.JPG</div>
                <span className="text-xs text-gray-500 mb-2">With Background</span>
                <div className="bg-indigo-100 text-indigo-800 text-xs px-2 py-1 rounded-full">Smaller Size</div>
                <div className="mt-2 text-xs text-gray-600 text-center">Best for photos, email</div>
              </div>
            </div>
          </div>
          
          <div className="mb-6">
            <h4 className="text-sm font-semibold text-indigo-700 mb-2">Size Options</h4>
              ...
              {Object.entries(sizeMap).map(([size, label]) => (
                <div 
                  ...
                  className={`border rounded-lg p-3 bg-white hover:shadow-md transition-all cursor-pointer text-center
                    ${selectedSize === size ? 'border-indigo-400 shadow-md' : 'border-indigo-200 hover:border-indigo-400'}`}
